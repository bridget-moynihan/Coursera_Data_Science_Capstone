...
def get_scatter_chart(entered_site, payload_slider):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= int(payload_slider[0])) &
                                (spacex_df['Payload Mass (kg)'] <= int(payload_slider[1]))
                               ]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y="class")
        return fig
    else:
        filtered_data = spacex_df[(spacex_df['Launch Site'] == entered_site) & 
                                (spacex_df['Payload Mass (kg)'] >= int(payload_slider[0])) &
                                (spacex_df['Payload Mass (kg)'] <= int(payload_slider[1]))
                               ]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y="class")
        return fig
